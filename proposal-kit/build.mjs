...
function renderExamples(examples = [], clientName) {
  ...
  return examples.map((example) => `
          <a class="example-card reveal" href="${escapeHtml(example.url)}" target="_blank" rel="noreferrer" aria-label="View the ${escapeHtml(example.name)} live website">
            <div class="example-image"><img src="${escapeHtml(example.image)}" alt="${escapeHtml(example.alt || `Screenshot of the ${example.name} website`)}" loading="lazy"></div>
...
const style = String.raw`
    ...
    * { box-sizing: border-box; }
    ...
    .panel-label { color: var(--muted); font-size: 0.69rem; font-weight: 850; letter-spacing: 0.14em; text-transform: uppercase; }
    ...
    .card-index { color: var(--accent); font-size: 0.67rem; font-weight: 850; letter-spacing: 0.12em; }
    ...
    .feature-icon { align-items: center; background: var(--accent); border-radius: 5px; color: #000; display: flex; font-size: 0.67rem; font-weight: 900; height: 30px; justify-content: center; margin-bottom: 27px; width: 30px; }
    ...
    .ownership-footnote, .guarantee { color: var(--muted); font-size: 0.7rem; margin: 17px 0 0; }
    ...
    .pipeline-card small { color: var(--accent); display: block; font-size: 0.58rem; font-weight: 850; letter-spacing: 0.06em; margin-bottom: 15px; text-transform: uppercase; }
    .pipeline-card strong { display: block; font-size: 0.76rem; line-height: 1.16; }
    .pipeline-card span { color: var(--muted); display: block; font-size: 0.62rem; line-height: 1.28; margin-top: 6px; }
    .pipeline-caption { color: var(--muted); font-size: 0.76rem; margin: 17px 0 0; }
    ...
    footer { border-top: 1px solid var(--line); color: var(--muted); font-size: 0.7rem; padding: 20px 0 25px; }
  ...
  <main id="top">
    ...
    <section class="commercial" id="next-step">
      <div class="wrap commercial-layout">
        ...
        <aside class="price-card reveal">
          ...
          <a class="button button-primary" href="${escapeHtml(config.offer.ctaHref || '#contact')}">${escapeHtml(config.offer.ctaLabel || 'Talk through the setup')} <span aria-hidden="true">↗</span></a>
    ...
    <section class="final-cta">
      <div class="wrap reveal">
        ...
        <div class="button-row"><a class="button button-primary" href="${escapeHtml(config.final.primaryHref || '#next-step')}">${escapeHtml(config.final.primaryLabel || 'Start the conversation')} <span aria-hidden="true">↑</span></a><a class="button button-quiet" href="#top">Back to the top</a></div>
      </div>
    </section>
  </main>
